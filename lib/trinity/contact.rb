module Trinity
  class Contact
    ...
    def self.generate(kind)
      ...
      c = Trinity::Contacts.const_get(sym).new

      unless c.kind_of?(Contact)
        abort "Contact '#{c.class.name}' must subclass Trinity::Contact"
      end

      c
    rescue NameError
      raise NoSuchContactError.new("No Contact found with the class name Trinity::Contacts::#{sym}")
    end
