...
module Trinity
  module Contacts
    ...
    class Email < Contact
      ...
      self.from_email = 'trinity@example.com'
      self.from_name = 'Trinity'
...
Category: #{category}
      ...
      def notify(message, time, priority, category, host)
        ...
        logmsg(:info, "failed to send email to #{arg(:to_email)} via #{arg(:delivery_method).to_s}: #{e.message}")
      ...
      def notify_smtp(mail)
        ...
        smtp = Net::SMTP.new arg(:server_host), arg(:server_port).to_i
        smtp.enable_starttls
        smtp.start(*args) do |s|
          s.send_message(mail, arg(:from_email), arg(:to_email))
        end
      end
